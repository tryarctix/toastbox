...
(() => {
  ...
  const themes = {
    classic: { base: "flex items-center gap-3 bg-white text-black border p-4 rounded shadow relative", success: "border-green-500 text-green-800", error: "border-red-500 text-red-800", warning: "border-yellow-500 text-yellow-800", info: "border-blue-500 text-blue-800", icon: { success: "✅", error: "❌", warning: "⚠️", info: "ℹ️" } },
    flat:    { base: "flex items-center gap-3 bg-white text-black border-l-4 p-4 rounded shadow-sm relative", success: "border-green-500", error: "border-red-500", warning: "border-yellow-500", info: "border-blue-500", icon: { success: "✔️", error: "🛑", warning: "⚠️", info: "💡" } },
    glass:   { base: "flex items-center gap-3 bg-white/30 backdrop-blur border border-white/30 text-black p-4 rounded shadow relative", success: "border-green-300 text-green-900", error: "border-red-300 text-red-900", warning: "border-yellow-300 text-yellow-900", info: "border-blue-300 text-blue-900", dark: { success: "text-green-300 border-green-500", error: "text-red-300 border-red-500", warning: "text-yellow-300 border-yellow-500", info: "text-blue-300 border-blue-500" }, icon: { success: "🧊", error: "💥", warning: "🌫️", info: "💡" } },
    neon:    { base: "flex items-center gap-3 bg-black border text-green-400 p-4 rounded shadow relative", success: "border-green-500 text-green-400", error: "border-pink-500 text-pink-400", warning: "border-yellow-500 text-yellow-400", info: "border-cyan-500 text-cyan-400", icon: { success: "🟢", error: "🟥", warning: "🟡", info: "🔵" } },
    material:{ base: "flex items-center gap-4 p-4 rounded shadow bg-white relative", success: "border-l-4 border-green-500 text-green-800", error: "border-l-4 border-red-500 text-red-800", warning: "border-l-4 border-yellow-500 text-yellow-800", info: "border-l-4 border-blue-500 text-blue-800", icon: { success: "✔️", error: "✖️", warning: "⚠️", info: "ℹ️" } },
    bubble:  { base: "flex items-center gap-3 bg-white border p-4 rounded-full shadow relative", success: "border-green-400 text-green-600", error: "border-red-400 text-red-600", warning: "border-yellow-400 text-yellow-600", info: "border-blue-400 text-blue-600", icon: { success: "🥤", error: "💣", warning: "📣", info: "🔔" } },
    retro:   { base: "flex items-center gap-3 bg-yellow-50 border border-yellow-800 text-yellow-900 p-4 rounded shadow relative font-mono", success: "border-green-700 text-green-800", error: "border-red-700 text-red-800", warning: "border-yellow-700 text-yellow-900", info: "border-blue-700 text-blue-900", icon: { success: "📟", error: "📼", warning: "📻", info: "📠" } },
    gradient:{ base: "flex items-center gap-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white p-4 rounded shadow relative", success: "", error: "", warning: "", info: "", icon: { success: "🌈", error: "🔥", warning: "⚡", info: "☁️" } },
    solid:   { base: "flex items-center gap-3 p-4 rounded text-white shadow relative", success: "bg-green-600", error: "bg-red-600", warning: "bg-yellow-500 text-black", info: "bg-blue-600", icon: { success: "✅", error: "❌", warning: "⚠️", info: "ℹ️" } },
    minimal: { base: "flex items-center gap-3 text-sm p-3 border-l-4 bg-white shadow-sm relative", success: "border-green-500 text-green-800", error: "border-red-500 text-red-800", warning: "border-yellow-500 text-yellow-800", info: "border-blue-500 text-blue-800", icon: { success: "✔️", error: "✖️", warning: "⚠️", info: "ℹ️" } },
    cyberpunk: { base: "flex items-center gap-3 p-4 rounded shadow bg-black text-pink-500 border border-pink-400", success: "text-green-400 border-green-300", error: "text-red-400 border-red-300", warning: "text-yellow-400 border-yellow-300", info: "text-cyan-400 border-cyan-300", icon: { success: "🧬", error: "💀", warning: "⚡", info: "🤖" } },
    pastel:   { base: "flex items-center gap-3 p-4 rounded shadow bg-pink-50 border text-pink-700", success: "border-green-200 text-green-700", error: "border-red-200 text-red-700", warning: "border-yellow-200 text-yellow-700", info: "border-blue-200 text-blue-700", icon: { success: "🍃", error: "🍓", warning: "🍋", info: "🫐" } },
  ...
  window.initSuperToasts = function(messages, options = {}) {
    ...
    if (!c) {
      ...
      c.id = cid;
      c.className = `fixed z-50 space-y-3 w-full max-w-xs ${positions[position]}`;
    ...
    messages.forEach(({ message, tags }) => {
      let t = "info";
      if (tags.includes("success")) t = "success";
      else if (tags.includes("error")) t = "error";
      else if (tags.includes("warning")) t = "warning";

      const icon = cfg.icon[t] || "";
      const toast = document.createElement("div");
      toast.className = `${cfg.base} ${cfg[t]} ${darkMode && cfg.dark?.[t] ? cfg.dark[t] : ""} transform ${startAnim} transition-all duration-500 toast`;
      ...
      toast.querySelector("button")?.addEventListener("click", () => {
        toast.classList.add("toast-exit"); setTimeout(() => toast.remove(), 400);
      });
      onClick && toast.addEventListener("click", () => onClick({ type: t, message }));
      c.appendChild(toast);

      setTimeout(() => toast.className = toast.className.replace(startAnim, endAnim), 20);
      setTimeout(() => { toast.classList.add("toast-exit"); setTimeout(() => toast.remove(), 400); }, duration);
    });
  };
})();
</script>
